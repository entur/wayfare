...
// A package with several travellers has one ticket group per traveller, and
// stacking them all vertically makes the page huge. Showing one at a time
// with prev/next + dots keeps each ticket full-size and scannable.
function DocumentCarousel({ groups }: { groups: TicketGroup[] }) {
	...
	return (
		<div className="flex flex-col gap-3">
			<TicketGroupCard group={activeGroup} />
			{groups.length > 1 && (
				<div className="flex items-center justify-center gap-4">
					<button
						...
						className="flex h-8 w-8 items-center justify-center rounded-full border border-wayfare-line bg-wayfare-bg text-wayfare-text-secondary transition-colors hover:bg-wayfare-surface disabled:pointer-events-none disabled:opacity-30"
					>
					...
					<div className="flex items-center gap-1.5">
						{groups.map((group, i) => (
							<button
								...
								type="button"
								onClick={() => setIndex(i)}
								aria-label={`Show ticket ${i + 1} of ${groups.length}`}
								aria-current={i === activeIndex}
								className={`h-2 w-2 rounded-full transition-colors ${
									i === activeIndex ? "bg-wayfare-primary" : "bg-wayfare-line"
								}`}
							/>
						))}
					</div>
...
export default function DocumentViewer({ documents }: DocumentViewerProps) {
	...
	return <DocumentCarousel groups={groupDocuments(documents)} />;
}
